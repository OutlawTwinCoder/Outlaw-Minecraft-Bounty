...
public class BountyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(ChatColor.YELLOW + "/bounty shop");
            sender.sendMessage(ChatColor.YELLOW + "/bounty points");
            return true;
        }
        switch (args[0].toLowerCase()) {
            ...
            case "points" -> {
                if (sender instanceof Player p) {
                    int points = plugin.pointsManager().getPoints(p.getUniqueId());
                    p.sendMessage(ChatColor.GOLD + plugin.locale().tr("messages.points_balance", java.util.Map.of("points", String.valueOf(points))));
                }
            }
            default -> {
                return false;
